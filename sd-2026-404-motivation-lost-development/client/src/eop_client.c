...
eop_client_t* eop_connect(const char* host, int port, int timeout_ms)
{
    ...
    if (inet_pton(AF_INET, host, &server_ip) == 0)
    {
        set_global_error(EOP_ERR_INVALID_ARGUMENT, "Invalid host address");
        free(client);
        return NULL;
    }
    ...
    int connect_result = connect(client->socket_fd, (struct sockaddr*)&server_addr, sizeof(server_addr));
...
eop_error_code eop_reconnect(eop_client_t* client, int timeout_ms)
{
    ...
    struct in_addr server_ip;
    if (inet_pton(AF_INET, client->host, &server_ip) == 0)
    {
        set_client_error(client, EOP_ERR_INVALID_ARGUMENT, "Invalid stored host address");
        pthread_mutex_unlock(&client->mutex);
        return EOP_ERR_INVALID_ARGUMENT;
    }
    ...
    int connect_result = connect(new_fd, (struct sockaddr*)&server_addr, sizeof(server_addr));
    if (connect_result < 0 && errno != EINPROGRESS)
    {
