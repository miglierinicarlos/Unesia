...
Requires a running cluster and both images built locally.

```bash
# Build images
docker build -t eop-server:latest .
docker build -f Dockerfile.hpc -t eop-hpc-engine:latest .

# Load into minikube (if using a local cluster)
minikube image load eop-server:latest
minikube image load eop-hpc-engine:latest

# Validate manifests, deploy full stack, and wait for Ready (timeout: 3 min)
make k8s-apply
...
`make k8s-apply` runs `kubeconform` validation before applying — invalid manifests abort before any `kubectl apply`.

Manifests in `k8s/` include: Deployment with liveness/readiness probes,
Service, ConfigMap for environment-based configuration, and a Job for
one-shot batch analysis.

#### HPC engine — Deployment mode (continuous)

The HPC engine Deployment runs continuously, reconnecting to eop-server
and processing `ANALYZE_GRAPH` requests as they arrive. It is included in
`make k8s-apply` — no separate step needed.

#### HPC engine — Job mode (one-shot batch analysis)

The Job manifest runs the HPC engine in batch mode (`HPC_MODE=batch`): it
connects to eop-server, processes a single `ANALYZE_GRAPH` request, and
exits. Completed pods are auto-cleaned after 300 s (`ttlSecondsAfterFinished`).
...
```bash
make validate-k8s               # lint all manifests with kubeconform
make k8s-apply                  # validate + deploy full stack, wait for Ready
make k8s-hpc-job                # validate + run one-shot batch analysis Job
make test-k8s-rolling-restart   # zero-downtime rolling restart verification (AC7)
